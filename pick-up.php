...
// Check if the pick-up request has been made and set a session variable
if (isset($_POST['requestPickup'])) {
  $_SESSION['pickup_request'] = "passenger " . $_SESSION['user_name'] . " - pick up at location";
  // Redirect to the same page to avoid form resubmission on refresh
  header("Location: pick-up.php");
  exit();
}
...
<body>
  ...
  <section class="page-content">
    <section class="grid">
      ...
      <article class="bottom-grid" style="display:flex; flex-direction:column; text-align: center; height:100%;">

        <div class="text"
          style="text-align: left; margin-right: 10px;  gap: 20px; margin-top: 20px; margin-bottom: 30px;">
          <h2>Request Pick-Up</h2>
          <p>Request for a pick up! An appropriate location will be displayed for you on the map below to travel to for the bus to collect you.</p>
        </div>

        <div class="container-pick-up">
          <h1>Would you like to request a pick-up?</h1>
          <p style="margin-bottom: 10px;">Click the button bellow</p>
          <img src="images/arrow.png" alt="arrow" style="height: 50px; width: 50px;">
          <form method="post">
            <button id="requestButton" name="requestPickup">Request Pick-Up</button>
          </form>
        </div>

        <iframe id="mapFrame"
          src="https://www.google.com/maps/d/u/3/embed?mid=1vU6rPlpaOQPaZMqPnOcAMfo0caDCkeM&ehbc=2E312F&noprof=1"
          width="800" height="480"></iframe>

        <button id="cancelButton">Cancel Pick-Up</button>
