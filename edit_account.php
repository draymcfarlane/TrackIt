...
// if change password button was pressed
if (isset($_POST['change_pass_btn'])) {
  ...
  if (!$uppercase || !$lowercase || !$number || !$specialChars || strlen($new_password) < 8) {
    ...
    $uid = $_POST['change_pwd_user_id']; // assigns uid of current user

    if ($new_password == $retype_password) {
      $updatedUser = $auth->changeUserPassword($uid, $new_password);
      if ($updatedUser) {
        header("Location: password_confirm.php"); // notify user of password change
      } else {
...
<head>
  ...
  <!-- determines what styling the page should be displayed in -->
  <?php
  $email = $_SESSION['user'];
  ...
  if ($fetchdata > 0) {
    foreach ($fetchdata as $key => $row) {
      ...
      foreach ($users as $user) {
        if ($user->email == $email) {
          // uses the bus driver styles if the account logged in is a bus driver
          if ($row['user_type'] == "bus driver" && $row['email'] == $email) {
            ?>
          ...
          <body>
            ...
            <header class="page-header"> <!--bus driver -->
              <nav>
            ...
            // uses the passenger styles if the account logged in is a passenger
          } elseif ($row['user_type'] == "passenger" && $row['email'] == $email) {
            ?>
            <link rel="stylesheet" href="css/passenger-dashboard.css">
            <link rel="stylesheet" href="css/passenger-style.css">

            <style>
              /* adjust the content to display in two columns */
              .page-content .grid {
                display: grid;
  ...
  ?>
    ...
    <div class="container">
      ...
      <div class="content">
        <h3>ky, <span>
            <?php // displays the appropriate user account role
            $ref_table = 'user/users';
            $fetchdata = $database->getReference($ref_table)->getValue();
              ...
              foreach ($fetchdata as $key => $row) {
                ...
                foreach ($users as $user) {
                  if ($user->email == $email) {
                    if ($row['user_type'] == "bus driver" && $row['email'] == $email) {
                      ?>bus driver
                    <?php } elseif ($row['user_type'] == "passenger" && $row['email'] == $email) {
                      ?>passenger
                    <?php }
                  }
                }
        ...
        <h1>welcome <span>
            <?php // displays the current logged in user's name
            include ("dbcon.php");
            $users = $auth->listUsers();
    ...
    <?php // finds the correct user account to change the password for
    include ('dbcon.php');
    $users = $auth->listUsers();
    $email = $_SESSION['user'];
    ...
    foreach ($users as $user) {
      ...
      if (count($data) == 0) {
        ...
        if ($fetchdata > 0) {
          foreach ($fetchdata as $key => $row) {
            if ($row['email'] == $email && $row['email'] == $user->email) { // once the account is found, displays the below
              ...
              <section class="page-content">
                <section class="grid" style="text-align: center;">
                    ...
                    <form method="post" action="">
                      ...
                      if (isset($_GET['id'])) {
                        ...
                        try {
                          ...
                          ?>
                          <input type="hidden" name="change_pwd_user_id" value="<?= $uid ?>"> <!-- stores the uid for passing -->
                          ...
                          ; ?>
                          <input type="password" name="newpassword" id="password" required placeholder="enter your new password">
                          <input type="password" name="retypepassword" id="cpassword" required
                            placeholder="re-enter your new password">
                          <span class="eye" onclick="doubleHidePassword()">
                            <i id="hide1" class="fa fa-eye"></i>
                            <i id="hide2" class="fa fa-eye-slash"></i>
                          </span>
                          <input type="submit" name="change_pass_btn" value="Change" class="form-btn">
                          <?php
                  ...
                  <div class="form-container">
                    <form method="post" action="delete_user_code.php">
                      <h3>Delete account</h3>

                      <input type="button" onclick="document.getElementById('id01').style.display='block'"
                        name="attempt_delete_user" value="Delete" class="form-btn">
                      ...
                      <div id="id01" class="modal ">
                        ...
                        <form method="post" action="">
                          <div class="mdcontainer">
                            ...
                            <input type="button" onclick="document.getElementById('id01').style.display='none'"
                              name="dont_delete_user" value="No" class="form-btn">

                            <input type="submit" onclick="document.getElementById('id01').style.display='none'" name="delete_user"
                              value="Yes" class="form-btn">
                            <input type="hidden" name="dbdelete" value="<?= $user->uid; ?>">
                            <input type="hidden" name="rtdbdelete" value="<?php echo $key; ?>">
                          </div>
                        </form>
                      </div>
